...
def _input_process_func(field: pa.Field) -> Callable:
    ...
    if pa.types.is_list(field.type) or pa.types.is_fixed_size_list(field.type):
        ...
        return lambda array: (
            [func(v) if v is not None else None for v in array]
    ...
    if pa.types.is_map(field.type):
        ...
        # list[tuple[k,v]] -> dict
        return lambda array: (
            dict(tuple(func(v) for v, func in zip(item, funcs)) for item in array)
            if array is not None
            else None
        )
...
def _output_process_func(field: pa.Field) -> Callable:
    ...
    if pa.types.is_struct(field.type):
        ...
        return lambda tup: (
            tuple(func(v) if v is not None else None for v, func in zip(tup, funcs))
            if tup is not None
            else None
        )
    if pa.types.is_map(field.type):
